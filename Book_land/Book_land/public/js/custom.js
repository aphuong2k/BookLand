		...
		if($("input[name='demo_vertical2']").length > 0 ) {
			jQuery("input[name='demo_vertical2']").TouchSpin({
			  ...
			  min:1
				...
				var a = Number($(this).closest('td').parent()[0].sectionRowIndex) + 1
				var quantity = document.getElementById("tabela").rows[a].cells[4].querySelector('input').value;
				var id = document.getElementById("tabela").rows[a].cells[4].querySelector('.id').value;
				var price = document.getElementById("tabela").rows[a].cells[3].innerText;
				//document.getElementById("total").rows[0].cells[1].innerHTML = total
				document.getElementById("tabela").rows[a].cells[5].innerHTML = Number(quantity) * Number(price)
				check()
				$.ajax({
                    url: "/update-quantity",
                    datatype: "text",
